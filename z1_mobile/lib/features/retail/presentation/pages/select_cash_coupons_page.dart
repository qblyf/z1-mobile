...
/// 代金券选择页面
/// 用于零售开单中选择可用的代金券
class SelectCashCouponsPage extends StatelessWidget {
  const SelectCashCouponsPage({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (ctx) => CashCouponBloc(
        dataSource: CashCouponRemoteDataSourceImpl(
          apiClient: ctx.read<dynamic>(),
        ),
      )..add(const CashCouponLoadRequested()),
      child: const _SelectCashCouponsView(),
    );
  }
}

class _SelectCashCouponsView extends StatelessWidget {
  const _SelectCashCouponsView();

  @override
  Widget build(BuildContext context) {
    return CupertinoPageScaffold(
      navigationBar: CupertinoNavigationBar(
        middle: const Text('选择代金券'),
      ...
      child: SafeArea(
        child: BlocBuilder<CashCouponBloc, CashCouponState>(
          builder: (context, state) {
            if (state is CashCouponLoading) {
              return const Center(child: CupertinoActivityIndicator());
            }
            if (state is CashCouponError) {
              return Center(
                child: Column(
                  ...
                  children: [
                    ...
                    CupertinoButton(
                      child: const Text('重试'),
                      onPressed: () =>
                          context.read<CashCouponBloc>().add(const CashCouponLoadRequested()),
                    ),
                  ],
                ),
              );
            }
            if (state is CashCouponLoaded) {
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (ctx, index) {
                        ...
                        return _CashCouponCard(
                          coupon: coupon,
                          isSelected: isSelected,
                          onTap: () {
                            context.read<CashCouponBloc>().add(
                                  CashCouponSelectionToggled(coupon),
                                );
                          },
                        );
                      },
  ...
  Widget _buildBottomBar(BuildContext context, CashCouponLoaded state) {
  ...
  void _onConfirm(BuildContext context) {
    final state = context.read<CashCouponBloc>().state;
    if (state is CashCouponLoaded) {
...
class _CashCouponCard extends StatelessWidget {
  final CashCouponModel coupon;
  final bool isSelected;
  final VoidCallback onTap;

  const _CashCouponCard({
